/**
 * Agent Worker Loop — Phase 5
 *
 * Orchestrates the full lifecycle of one unit of work:
 *   dispatch → spawn agent → validate → commit → PR → complete
 *
 * The "spawn agent" step invokes `claude` CLI (Claude Code) with the
 * work context as a prompt. On non-Claude lanes (or when CLI is absent)
 * it falls back to a no-op stub so the rest of the pipeline still runs.
 *
 * Depends on: queue-manager, agent-slot-manager, work-dispatcher, worktree-manager
 */

"use strict";

const fs              = require("fs");
const path            = require("path");
const { execSync, execFileSync, spawnSync } = require("child_process");
const QueueManager    = require("./queue-manager");
const AgentSlotManager = require("./agent-slot-manager");
const { dispatchOne } = require("./work-dispatcher");
const { removeWorktree } = require("./worktree-manager");

const REPO_ROOT    = path.resolve(__dirname, "..");
const MAX_AGENT_MS = parseInt(process.env.AGENT_TIMEOUT_MS || "300000", 10);
const _queue       = new QueueManager(path.join(REPO_ROOT, "data", "agent-work-queue"));
const _slots       = new AgentSlotManager();

// ── Git helpers ───────────────────────────────────────────────────────────────

// Shell-free: git args are passed as a discrete argv array to execFileSync
// (shell:false), so a commit message or branch name is never re-interpreted by
// a shell. Callers pass arrays, e.g. git(["commit", "-m", msg]).
function git(args, cwd = REPO_ROOT) {
  return execFileSync("git", args, { cwd, encoding: "utf8" }).trim();
}

function gitSafe(args, cwd = REPO_ROOT) {
  try { return git(args, cwd); } catch (e) { return null; }
}

// ── Test runner ───────────────────────────────────────────────────────────────

// #896: classify a pytest run into {passed, inconclusive}. A run with NO tests
// collected (pytest exit 5 / "no tests ran" / "collected 0 items") is NOT a pass —
// it is inconclusive, and the caller must treat it like a failure rather than
// award verification. `passed` requires a positive "<N> passed" signal AND no
// failure marker; absence of any signal is inconclusive, never success.
function classifyTestOutput(out) {
  const text = String(out || "");
  const failed = /\bFAILED\b|\bERROR(S)?\b|\b\d+ failed\b|\b\d+ error/i.test(text);
  const noTests = /no tests ran|collected 0 items/i.test(text);
  const passedSignal = /\b\d+ passed\b/.test(text);
  if (failed) return { passed: false, inconclusive: false };
  if (noTests || !passedSignal) return { passed: false, inconclusive: true };
  return { passed: true, inconclusive: false };
}

function runTests(worktreePath) {
  let out = "";
  try {
    out = execSync(
      "python -m pytest tests/ -q --tb=short " +
        "--ignore=tests/test_anti_entropy_memory.py " +
        "--ignore=tests/test_audit_chain.py " +
        "--ignore=tests/test_discord_bot.py " +
        "--ignore=tests/test_discord_voice_gate.py",
      { cwd: worktreePath, encoding: "utf8", timeout: 120_000 }
    );
  } catch (e) {
    // pytest exits non-zero on failures (1) AND on no-tests-collected (5) — capture
    // stdout in both cases and let classifyTestOutput tell them apart.
    out = String(e.stdout || "") + String(e.stderr || e.message || "");
  }
  const cls = classifyTestOutput(out);
  return { passed: cls.passed, inconclusive: cls.inconclusive, output: out.slice(-500) };
}

// ── PR creation ───────────────────────────────────────────────────────────────

const GH_REPO = process.env.GH_REPO || "alex-place/lantern-os";

// Owner of the `origin` remote (where pushBranch actually pushes), for
// `gh pr create --head <owner>:<branch>`. The PR head MUST track the push remote —
// hardcoding a fork owner that isn't `origin` makes gh 422 ("head not found"). When
// origin == the base-repo owner (same-repo push, the common case here) the owner
// prefix is omitted. Override with AUTOWORK_FORK_OWNER. #871
function originOwner(cwd = REPO_ROOT) {
  if (process.env.AUTOWORK_FORK_OWNER) return process.env.AUTOWORK_FORK_OWNER;
  const url = gitSafe(["remote", "get-url", "origin"], cwd);   // https://github.com/<owner>/<repo>.git | git@github.com:<owner>/<repo>.git
  const m = url && url.match(/[/:]([^/]+)\/[^/]+?(?:\.git)?$/);
  return (m && m[1]) || null;
}

// The `gh pr create --head` value: <fork-owner>:<branch> when origin is a fork,
// bare <branch> when origin == the base repo. Pure (no shell) so it's unit-testable.
function prHead(branch) {
  const baseOwner = GH_REPO.split("/")[0];
  const owner = originOwner();
  return (owner && owner !== baseOwner) ? `${owner}:${branch}` : branch;
}

function createPR(branch, issueNumber, title) {
  try {
    const body = `Closes #${issueNumber}\n\nGenerated by Claude lane worker loop.\n\n🤖 Generated with [Claude Code](https://claude.com/claude-code)`;
    const head = prHead(branch);
    // Shell-free: the issue `title`/`body` reach gh as discrete argv elements, so
    // a title containing `$(…)`/backticks/`;` can't inject a command (it did under
    // the old shell string, despite the JSON.stringify quoting).
    const out = execFileSync(
      "gh",
      ["pr", "create", "--repo", GH_REPO, "--head", head, "--base", "master",
       "--title", `fix: ${title}`, "--body", body],
      { cwd: REPO_ROOT, encoding: "utf8", timeout: 30_000 }
    ).trim();
    const url = out.match(/https:\/\/github\.com\/\S+/)?.[0] || out;
    return { ok: true, url };
  } catch (e) {
    return { ok: false, error: e.message.slice(0, 300) };
  }
}

// ── Agent invocation ──────────────────────────────────────────────────────────

function spawnClaudeAgent(worktreePath, context) {
  // Check if claude CLI is available
  const whichClaude = spawnSync("where", ["claude"], { encoding: "utf8", shell: true });
  if (whichClaude.status !== 0) {
    return { ok: false, skipped: true, reason: "claude CLI not found — manual work required" };
  }

  const prompt = [
    `You are working on: ${context.title}`,
    `Issue: ${context.issue_url}`,
    "",
    context.body_excerpt || "",
    "",
    "Read the issue, make the necessary code changes, run any tests, and commit your work.",
    "Keep commits small and focused. Do not push — the system will handle that.",
  ].filter(s => s !== undefined).join("\n");

  const result = spawnSync("claude", ["--print", "--dangerously-skip-permissions", prompt], {
    cwd:      worktreePath,
    encoding: "utf8",
    timeout:  MAX_AGENT_MS,
    shell:    true,
  });

  return {
    ok:     result.status === 0,
    stdout: (result.stdout || "").slice(0, 2000),
    stderr: (result.stderr || "").slice(0, 500),
    status: result.status,
  };
}

// ── Commit any changes the agent made ────────────────────────────────────────

function commitAgentWork(worktreePath, issueNumber, title) {
  const dirty = gitSafe(["status", "--short"], worktreePath);
  if (!dirty || dirty.trim() === "") return { committed: false, reason: "no changes" };

  gitSafe(["add", "-A"], worktreePath);
  const msg = `fix: ${title.slice(0, 60)} (issue #${issueNumber})\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>`;
  try {
    git(["commit", "-m", msg], worktreePath);
    return { committed: true };
  } catch (e) {
    return { committed: false, reason: e.message.slice(0, 200) };
  }
}

function pushBranch(worktreePath, branch) {
  try {
    git(["push", "origin", branch], worktreePath);
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e.message.slice(0, 200) };
  }
}

// ── Main worker loop ──────────────────────────────────────────────────────────

/**
 * Process one work item end-to-end.
 * Returns a receipt object summarising what happened.
 */
async function processOne(lane, opts = {}) {
  const workItem = await dispatchOne(lane, opts);
  if (!workItem) return null;

  const { entry, slot, worktreePath, branch, context } = workItem;
  const receipt = {
    entry_id:     entry.id,
    issue_number: entry.issue_number,
    slot_id:      slot.id,
    branch,
    worktreePath,
    steps:        [],
    ok:           false,
  };

  const step = (name, result) => {
    receipt.steps.push({ name, ...result });
    try { _slots.heartbeat(slot.id); } catch {}
    return result;
  };

  const issueNum = entry.issueNumber || entry.issue_number;
  const title    = entry.title || `issue #${issueNum}`;

  try {
    // 1. Spawn agent
    const agentResult = step("agent", worktreePath
      ? spawnClaudeAgent(worktreePath, context)
      : { ok: false, skipped: true, reason: "no worktree" });

    if (!agentResult.ok && !agentResult.skipped) {
      step("fail", { ok: false, reason: "agent failed" });
      await _queue.markFailed(entry.id, agentResult.stderr || agentResult.reason || "agent failed");
      try { _slots.completeWork(slot.id, { workId: entry.id, failed: true }); } catch {}
      receipt.ok = false;
      return receipt;
    }

    // No worktree → nothing could have been done; don't mark complete. #870
    if (!worktreePath) {
      step("incomplete", { ok: false, reason: "no worktree — no work performed" });
      await _queue.markFailed(entry.id, "no worktree — no work performed");
      try { _slots.completeWork(slot.id, { workId: entry.id, failed: true }); } catch {}
      receipt.ok = false;
      return receipt;
    }

    // 2. Commit changes
    const commitResult = step("commit", commitAgentWork(worktreePath, issueNum, title));

    // No diff committed (agent skipped — e.g. CLI absent — or made no changes) means
    // the issue was NOT worked. Requeue rather than falsely marking it complete. #870
    if (!commitResult.committed) {
      const reason = agentResult.skipped
        ? (agentResult.reason || "agent skipped — no work performed")
        : (commitResult.reason || "no changes committed");
      step("incomplete", { ok: false, reason });
      await _queue.markFailed(entry.id, reason);
      try { _slots.completeWork(slot.id, { workId: entry.id, failed: true }); } catch {}
      receipt.ok = false;
      return receipt;
    }

    // 3. Run tests — and GATE push/PR/complete on the result (mirror the live
    // dispatch path). A failing suite rolls back the commit and requeues. #870
    const testResult = step("tests", runTests(worktreePath));
    if (!testResult.passed) {
      step("rollback", { ok: false, reason: "tests_failed" });
      gitSafe(["reset", "--hard", "HEAD~1"], worktreePath); // drop the just-made (broken) commit
      await _queue.markFailed(entry.id, "tests failed — changes rolled back");
      try { _slots.completeWork(slot.id, { workId: entry.id, failed: true }); } catch {}
      receipt.ok = false;
      receipt.stoppedAt = "tests_failed";
      return receipt;
    }

    // 4. Push branch
    const pushResult = step("push", pushBranch(worktreePath, branch));
    // 5. Create PR (only if push succeeded)
    if (pushResult.ok) {
      const prResult = step("pr", createPR(branch, issueNum, title));
      if (prResult.ok) receipt.pr_url = prResult.url;
    }

    // 6. Complete queue entry — only reached when work was committed AND tests passed.
    receipt.ok = true;
    await _queue.markComplete(entry.id, receipt);
    try { _slots.completeWork(slot.id, { workId: entry.id, duration: Date.now() - new Date(entry.assignedAt || Date.now()).getTime() }); } catch {}

    // 7. Cleanup worktree
    if (worktreePath && !opts.keepWorktree) {
      try { removeWorktree(worktreePath, { deleteBranch: false }); } catch {}
    }

  } catch (err) {
    step("error", { ok: false, reason: err.message });
    try { await _queue.markFailed(entry.id, err.message); } catch {}
    try { _slots.completeWork(slot.id, { workId: entry.id, failed: true }); } catch {}
  }

  return receipt;
}

/**
 * Run the worker loop continuously until the queue is empty.
 */
async function runLoop(lane, opts = {}) {
  const receipts = [];
  let receipt;
  while ((receipt = await processOne(lane, opts)) !== null) {
    receipts.push(receipt);
    if (opts.onComplete) opts.onComplete(receipt);
  }
  return receipts;
}

module.exports = { processOne, runLoop, runTests, commitAgentWork, createPR, prHead, originOwner };
