...
use csf::{CompressionMode, Compressor, Decompressor, SecurityPolicy};
...
fn bench_compress_json(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("csf", "v1.0-full"), |b| {
        b.iter(|| {
            let mut comp = Compressor::new(policy.clone());
            let _ = comp
                .compress_text_mode(black_box(&text), CompressionMode::Full)
                .unwrap();
        });
    });
    ...
    group.bench_function(BenchmarkId::new("csf", "v1.0-fast"), |b| {
        b.iter(|| {
            let mut comp = Compressor::new(policy.clone());
            let _ = comp
                .compress_text_mode(black_box(&text), CompressionMode::Fast)
                .unwrap();
        });
    });
