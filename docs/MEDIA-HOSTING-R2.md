...
## All binary media off git-LFS → R2 (2026-06-30)

The repo's Git-LFS budget was exhausted (clones/checkouts aborted with a smudge
error — including the Colab notebook and the stable auto-deploy). Fix: **all** raster
media + PDFs are now out of git entirely and served from R2.

- `.gitattributes` no longer LFS-tracks `*.png/jpg/jpeg/gif/pdf/zip`; `.gitignore`
  ignores those (plus `*.webp/*.mp4`). SVGs stay in git (text/vector). The 76 LFS
  files were `git rm --cached` (kept on local disk, removed from git history-forward).
- Why no history rewrite: git-LFS only smudges *checked-out* files, so dropping them
  from HEAD already stops fresh clones from pulling them. Safe on this multi-agent repo.
- `scripts/sync-media-r2.sh` now also uploads the media trees (`data/images`, `images`,
  `reports`, `dollhouse`, `caadi-images`) to matching R2 prefixes.
- Web refs use R2: e.g. three-doors images via `THREE_DOORS_MEDIA_BASE`
  (`media.lantern-os.net/data/images/three-doors/`), overridable with
  `window.MEDIA_BASE_THREE_DOORS` for local dev.

### ⚠️ Ordering (do this or the live site 404s)

The stable 4177 worktree auto-resets to `master`, so once this merges it **loses the
local image files** (they're no longer in git). Therefore:

1. **Upload first** — `scripts/sync-media-r2.sh` (dry-run) then `--go`, from a checkout
   that still has the real bytes (the main checkout — it does).
2. **Confirm** an image resolves, e.g. open
   `https://media.lantern-os.net/data/images/three-doors/burrow.png`.
3. **Then merge** the migration PR.

Until the upload lands, keep the PR unmerged. (Bonus: removing LFS also un-wedges the
stable deploy's smudge-abort.)
