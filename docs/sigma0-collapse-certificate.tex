...
($28$ passing, $1$ xfail pending \#506) for the symmetric case. The other components are heuristics,
...
\textbf{Recommended:} report both $\alpha$ and $\max\Real\lambda(A)$.
\texttt{collapse\_certificate()} currently computes only the former.
...
\item \texttt{src/cio\_sde/collapse.py} --- \texttt{SemanticCollapseOperator},
  ...
  \texttt{ReconstructionOperator}.
\item \texttt{src/cio\_sde/surprise.py} --- \texttt{SurpriseMonitor} and the Kalman
  measurement update (the missing inference step).
\item \texttt{tests/test\_cio\_sde.py} --- 28 passing, 1 xfail (surprise$\leftrightarrow\Sigma_0$
  integration pending \#506); Theorem~1 exercised in the symmetric case only.
