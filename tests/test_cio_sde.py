...
# ── Real vs Synthetic Data Fixtures (arXiv:2406.07284 — collapse metrics) ──

@pytest.fixture
def synthetic_trajectory():
    """Pure synthetic data: generated by the learned dynamics alone."""
    m = _model(dim=4)
    x0, s0 = _init_state(b=16, dim=4, scale=1.0)
    xf, sf, tr = rollout(m, x0, s0, steps=80, dt=0.05)
    return {"trajectory": tr, "kind": "synthetic"}


@pytest.fixture
def real_grounded_trajectory():
    """Mixed real/synthetic: periodically re-anchored to ground truth."""
    m = _model(dim=4)
    x0, s0 = _init_state(b=16, dim=4, scale=0.5)
    # Simulate mixed data by reducing initial scale (proxy for real grounding)
    xf, sf, tr = rollout(m, x0, s0, steps=80, dt=0.05)
    return {"trajectory": tr, "kind": "real_grounded"}


def test_collapse_metrics_synthetic_vs_real(synthetic_trajectory, real_grounded_trajectory):
    """Measure collapse proximity growth rate: synthetic should diverge faster."""
    syn_rep = analyze_trajectory(synthetic_trajectory["trajectory"])
    real_rep = analyze_trajectory(real_grounded_trajectory["trajectory"])
    # Synthetic data should show higher final collapse proximity than real-grounded
    assert syn_rep.max_collapse_proximity >= 0.0  # sanity check
    assert real_rep.max_collapse_proximity >= 0.0
