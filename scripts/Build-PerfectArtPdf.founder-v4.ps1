...
styles.add(ParagraphStyle(name="Callout", parent=styles["BodyText"], fontSize=8.8, leading=12, textColor=colors.HexColor("#1f2933")))
...
def draw_page_art(canvas, doc):
    ...
    for ridx, r in enumerate(layers):
        ...
        for i in range(12):
            ...
            y = cy + math.sin(a) * r
            ring.append((x, y))
            canvas.circle(x, y, 1.25 if ridx < 2 else 1.0, stroke=0, fill=1)
        points.append(ring)
    ...
    if line.startswith("|") and i + 1 < len(lines) and lines[i+1].startswith("|"):
        ...
        if rows:
            ...
            tbl.setStyle(TableStyle([
                ...
                ("GRID", (0,0), (-1,-1), 0.25, colors.HexColor("#89969e")),
                ("VALIGN", (0,0), (-1,-1), "TOP"),
