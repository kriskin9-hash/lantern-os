...
if ($RegisterAutostart) {
    ...
    # Wrapper script runs the full boot: git pull → HFF seed → node --watch
    $wrapperPath = Join-Path $LanternRoot "scripts\lantern-autostart.ps1"
    @"
# Auto-generated by Start-Lantern.ps1 -RegisterAutostart
# DO NOT EDIT — re-run -RegisterAutostart to regenerate
Set-Location "$LanternRoot"
New-Item -ItemType Directory -Force -Path "$logDir" | Out-Null
`$log = "$logDir\lantern-autostart.log"
"[`$(Get-Date -Format 'u')] Lantern autostart triggered" | Tee-Object -Append `$log
# Pull latest, seed HFF, start server (node --watch for auto-restart on file change)
...
"@ | Set-Content -Path $wrapperPath -Encoding UTF8
    ...
    # Start both at system boot and at every user login
    ...
    Write-Host "    Lantern will start on boot and every login — no hands needed." -ForegroundColor Gray
    Write-Host "    Boot sequence: git pull → HFF seed → node --watch server" -ForegroundColor Gray
    Write-Host "    Auto-restarts: unlimited (1 min backoff on crash)" -ForegroundColor Gray
    Write-Host "    Logs: $logDir\lantern-autostart.log" -ForegroundColor Gray
