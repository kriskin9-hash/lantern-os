...
styles.add(ParagraphStyle(name="Small", parent=styles["BodyText"], fontSize=7.2, leading=9.2))
styles.add(ParagraphStyle(name="Codeish", parent=styles["BodyText"], fontName="Courier", fontSize=7.2, leading=9.5))
styles.add(ParagraphStyle(name="Callout", parent=styles["BodyText"], fontSize=8.8, leading=12, textColor=colors.HexColor("#35506b")))
...
def draw_page_art(canvas, doc):
    width, height = letter
    canvas.saveState()
    canvas.setFillColor(colors.HexColor("#eaf3ff"))
    ...
    if visual_paths:
        ...
        canvas.roundRect(6.75 * inch, 0.55 * inch, 0.7 * inch, 0.7 * inch, 6, fill=0, stroke=1)
        draw_image_cover(canvas, art, 6.75 * inch, 0.55 * inch, 0.7 * inch, 0.7 * inch)
    ...
    if line.startswith("|") and i + 1 < len(lines) and lines[i+1].startswith("|"):
        ...
        if rows:
            ...
            tbl.setStyle(TableStyle([
                ...
                ("TEXTCOLOR", (0,0), (-1,0), colors.HexColor("#2f4e70")),
                ("GRID", (0,0), (-1,-1), 0.25, colors.HexColor("#a9bfd6")),
                ("VALIGN", (0,0), (-1,-1), "TOP"),
