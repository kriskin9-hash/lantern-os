...
class ServiceAutomator {
  ...
  createServiceRequest(serviceOffer, customerData) {
    ...
    const offer = this.getExistingOffers().find(o => o === serviceOffer);
  ...
  getServicePricing(serviceOffer) {
    ...
    const pricing = pricingMap[serviceOffer];
    if (pricing) {
      return pricing;
    }
...
`;
    ...
    // Update wallet state with retry mechanism for concurrent access
    const maxRetries = 3;
    let retryCount = 0;
    let walletUpdated = false;

    while (retryCount < maxRetries && !walletUpdated) {
      try {
        const walletState = this.readWalletState();
        if (walletState) {
          ...
          const writeSuccess = this.writeWalletState(walletState);
          if (writeSuccess) {
            walletUpdated = true;
          } else {
            retryCount++;
        ...
        retryCount++;
        console.error(`Wallet state update failed (attempt ${retryCount}/${maxRetries}):`, error.message);
    ...
    if (!walletUpdated) {
      console.error('Failed to update wallet state after retries, but invoice was created');
    }
